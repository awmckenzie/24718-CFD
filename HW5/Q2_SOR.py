    ...
    while num_bad_nodes > 0:
        ...
        for l in range(1, Nx-1):
            for j in range(1, Ny-1):
                T_temp[l,j] = (1-omega) * T[l,j] + omega * (s * ((1/dx**2) * (T_temp[l-1,j] + T[l+1,j]) + (1/dy**2) * (T_temp[l,j-1] + T[l,j+1])))
                # T_temp[l,j] = (1-omega)*T[l,j] + omega*(T_temp[l-1,j] + T[l+1,j] + (dx**2/dy**2)*(T[l,j+1] + T_temp[l,j-1])) / (2 * (1 + (dx**2/dy**2)))
