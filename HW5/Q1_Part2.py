...
@jit
def scheme(dx, dy, Lx, Ly, u, v, mu, t, dt, dphi_x0, dphi_xLy, dphi_Lxy, phi_0y, IC):
    ...
    for n in range(t_steps-1):
        for l in range(1, x_int_nodes+1):
            for j in range(1, y_int_nodes+1):
                sol[n+1,l,j] = (sol[n,l,j] - (u * dt / dx)* (sol[n,l,j] - sol[n,l-1,j]) - (v * dt / dy) * (sol[n,l,j+1] - sol[n,l,j])
